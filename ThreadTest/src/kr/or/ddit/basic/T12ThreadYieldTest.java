...
class YieldThreadEx1 extends Thread {
	public YieldThreadEx1() {
		super("양보 스레드"); //부모(Thread)를 호출.
	}
	@Override
		public void run() {
			for(int i=0; i<10; i++) {
				System.out.println(Thread.currentThread().getName()  //getName() : 현재 스레드의 이름(양보 스레드)
									+ " : " + i);
				Thread.yield(); // 양보하기.
			}
		}
...
class YieldThreadEx2 extends Thread {
	public YieldThreadEx2() {
		super("비양보 스레드"); //부모(Thread)를 호출.
	}
	@Override
		public void run() {
			for(int i=0; i<10; i++) {
				System.out.println(Thread.currentThread().getName()  //getName() : 현재 스레드의 이름(비양보 스레드)
									+ " : " + i);
				// Thread.yield(); 가 없어서 양보를 안한다.
			}
		}
