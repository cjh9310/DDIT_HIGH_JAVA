...
public class MultiChatServer {
	...
	public void serverStart() {
		...
		try {
			...
			server = new ServerSocket(7777);  // 서버 생성
	...
	/**
	 * 서버에서 클라이언트로 메세지를 전송할 Thread클래스를 Inner클래스로 
	 * 정의한다. 
		...
		public ServerReceiver(Socket socket) {
			...
			public void run() {
				try {
					...
					// 수신해야 한다.
					name = dis.readUTF(); //name에 대화명 보관
					
					// 대화명을 받아서 다른 모든 클라이언트에게 대화방 참여
					// 메시지를 보낸다.
					sendMessage("#" + name  +"님이 입장했습니다."); 
					// Map에 있는 유저들에게 sendMessage를 단체로 뿌려줌
					...
					// 보내준다.
					while(dis != null) { // readUTF를 읽어서 sendMessage를 호출함.
				...
				//	ex.printStackTrace();   네트워크 끊어지면 빨간경고문이 막 뜨는 것.
				}finally {
