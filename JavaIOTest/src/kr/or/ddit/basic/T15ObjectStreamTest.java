...
public class T15ObjectStreamTest {
	public static void main(String[] args) {
		...
		Member mem2 = new Member("황도연",30,"경기");
		Member mem3 = new Member("정종운",40,"부산");
		Member mem4 = new Member("김진",50,"서울");
		...
		try {
			...
			// 출력용 스트림 객체 생성           //FileOutputStream 파일에 저장
		...
		try {
		...
		}catch(ClassNotFoundException ex) {   // ois.readObject()가 오류떠서 예외처리 해줌
			ex.printStackTrace();             // ois.readObject()가 오류떠서 예외처리 해줌
	 
		}catch(IOException ex) {
			//ex.printStackTrace();  
			System.out.println("출력완료");  //출력완료가 없고 그냥 예외처리하면 오류가 뜸.. 그
		}finally {
			try {
				ois.close();
			} catch (IOException e) {
		   ...
		   //implements Serializable를 지우고 실행해볼 것.
class Member implements Serializable {  
	// 자바는 Serializable 인터페이스를 구현한 클래스만 직렬화 할 수 있도록
	// 제한하고 있음.
	
	/*
	 	transient => 직렬화가 되지 않을 멤버변수에 지정한다.(직렬화 대상에서 제외시키고자 할때)
